...
function spawnExplosion(scene: THREE.Scene, pos: THREE.Vector3, out: Array<{m: THREE.Mesh, v: THREE.Vector3, life: number, spin: number}>) {
  ...
  for (let i = 0; i < 120; i++) {
    ...
    out.push({
      m, life: 0.8+Math.random()*0.6, spin: (Math.random()-0.5)*0.2,
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        // Spinning cube while loading so canvas goes live immediately
        ...
        const mkCone = (color: number, r: number, h: number, op: number) => {
          ...
          const c = new THREE.Mesh(new THREE.ConeGeometry(r, h, 12), mat)
        ...
        // Auto-detect nose axis and orient jet pointing up
        ...
        const expParts: Array<{m: THREE.Mesh, v: THREE.Vector3, life: number, spin: number}> = []
        ...
        const tick = () => {
          ...
          flames.forEach(({ mesh, baseOp }) => {
            ;(mesh.material as THREE.MeshBasicMaterial).opacity = baseOp*(0.72+Math.random()*0.28)
          ...
          if (phase === 'ground') {
            ...
            if (p >= 1) {
              ...
              spawnExplosion(scene, jet.position.clone(), expParts)
            ...
            for (const pt of expParts) {
              if (pt.life>0) {
                anyAlive=true; pt.life-=0.013; pt.m.position.add(pt.v); pt.v.y-=0.006
                pt.m.rotation.x+=pt.spin; pt.m.rotation.z+=pt.spin*0.7
                ;(pt.m.material as THREE.MeshBasicMaterial).opacity=Math.max(0,pt.life*1.25)
                pt.m.scale.setScalar(Math.max(0.01,1-(1-pt.life)*0.35))
